...
for (var i=0; i<data.length; i++) {
    ...
    db.warehouses.insert({
        ...
        "longitude": warehouse.location.long
    });
}
...
for (var i=0; i<data.length; i++) {
    ...
    for (var j=0; j<warehouse.cars.vehicles.length; j++) {
        var car = warehouse.cars.vehicles[j];

        db.cars.insert({
            "_id": car._id,
            "make": car.make,
            "model": car.model,
            "yearModel": car.year_model,
            "price": car.price,
            "licensed": car.licensed,
            "dateAdded": car.date_added,
            "warehouseId": warehouse._id,
            "warehouseLocation": warehouse.cars.location
        });
    }
}
